...
public class LEAVERequestAcceptor extends Thread {
    ...
    public LEAVERequestAcceptor(DatagramSocket datagramSocket, CopyOnWriteArrayList<Node> routingTable, Node myNode,
            String request) {
    ...
    public void run() {
        ...
        if (node != null) {
            ...
            System.out
                    .println("LeaveRequestAcceptor:inside send gossip method " + node.toString() + " " + node.getIpString());
            String msg = getFullMessage("LEAVEOK 0");
            bufToSend = msg.getBytes();
            System.out.println("LeaveRequestAcceptor:LEAVEOK Message:" + msg);
            DatagramPacket nodeDatagramPacket ;
            try {
                nodeDatagramPacket = new DatagramPacket(bufToSend, bufToSend.length, InetAddress.getByAddress(node.getIp()),
                        node.getPort());
                datagramSocket.send(nodeDatagramPacket);
                System.out.println(
                        "LeaveRequestAcceptor Message sent to " + node.getIpString() + " " + node.getPort() + " " + msg);
                routingTable.remove(node);
                System.out.println(
                        "LeaveRequestAcceptor:LeaveRequest  processed  removed node " + routingTable.size() + " " + node
                                .toString());
            ...
            System.out.println(
                    "LeaveRequestAcceptor:The node" + params[2] + " " + params[3] + " is not found in the routing table");
